...
def open_position_create_page(context):
    context.browser.get(f'{BASE_URL}kitchen/position/create/')
    time.sleep(6)
...
def submit_form(context):
    context.browser.find_element_by_css_selector('#savePosition').click().perform()
...
def should_be_at_position_list(context):
    ...
    assert context.browser.current_url == f'{BASE_URL}kitchen/position/list/'


@then(u'Я должен быть на странице создания позиции')
def should_be_at_position_create(context):
    assert context.browser.current_url == f'{BASE_URL}kitchen/position/create/'
