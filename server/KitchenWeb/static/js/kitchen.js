...
function addExtra(event){
	...
	$("<input/>").attr({type: 'text', id:'comment', placeholder:
        'Комментарий', class:'form-control w-75 p-3', name:('comment' + TYPES.length)}).appendTo('#selectBlock');
    $("<input/>").attr({type: 'number', id:'pricing', placeholder:
 ...
 function  addExtraBtn(event) {
	...
	if (TYPES.length > 1){
		 ...
		 $("<input/>").attr({type: 'text', id:'comment1', class:'form-control mt-4 w-75 p-3', placeholder:
        'Комментарий', name:('comment' + TYPES.length)}).appendTo('#selectBlock1');
    	 $("<input/>").attr({type: 'number', id:'pricing1', class:'form-control mt-4 w-75 p-3', placeholder:
