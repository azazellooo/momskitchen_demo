...
function addExtra(event){
	...
	$("<input/>").attr({type: 'text', id:'comment', value:
        'Комментарий', class:'form-control w-25 p-3', name:('comment' + TYPES.length)}).appendTo('#selectBlock');
    $("<input/>").attr({type: 'number', id:'pricing', value:
        '0', class:'form-control mt-4 w-25 p-3', name:('pricing' + TYPES.length)}).appendTo('#selectBlock');
 ...
 function  addExtraBtn(event) {
	...
	if (TYPES.length > 1){
		 ...
		 $("<input/>").attr({type: 'text', id:'comment1', class:'form-control mt-4 w-25 p-3', value:
        'Комментарий', name:('comment' + TYPES.length)}).appendTo('#selectBlock1');
    	 $("<input/>").attr({type: 'number', id:'pricing1', class:'form-control mt-4 w-25 p-3', value:
